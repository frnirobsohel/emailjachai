...
export function ApiKeysClient({ initialKeys }: { initialKeys: ApiKey[] }) {
    ...
    const onCreateKey = async (values: z.infer<typeof createKeySchema>) => {
        ...
        try {
            const data = await ApiClient.post('/user/keys/create', { name: values.name });
    ...
    const handleRotateKey = async (id: number) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                toast.success("API key rotated successfully", { id: toastId })
                fetchKeys();
            } else {
    ...
    const copyToClipboard = async (id: string | number, text?: string) => {
        if (!text) {
            toast.error("For security, existing API keys cannot be viewed again. Copy the key right after creating it.", { duration: 5000 });
            return
        }
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div className="space-y-3 pt-2">
                                <h4 className="text-xs font-bold text-slate-800 border-b border-slate-100 pb-1.5">Usage Statistics</h4>
                                <div className="space-y-2.5">
                                    <div className="flex justify-between items-center text-sm">
                                        ...
                                        <span className="font-bold text-slate-800">{(detailKey.credits_used || 0).toLocaleString()} Credits</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <div className="flex items-center gap-2 text-slate-500 font-medium">
                                        ...
                                        <span className="font-bold text-slate-800">{(detailKey.single_jobs || 0).toLocaleString()} Emails</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <div className="flex items-center gap-2 text-slate-500 font-medium">
                                        ...
                                        <span className="font-bold text-slate-800">{(detailKey.bulk_jobs || 0).toLocaleString()} Jobs</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <div className="flex items-center gap-2 text-slate-500 font-medium">
