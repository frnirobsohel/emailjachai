...
describe('applyClientIpHeaders', () => {
    it('sets forwarded + BFF/Cloudflare headers for Gin', () => {
        ...
        expect(target.get('X-EJP-Client-IP')).toBe('203.0.113.77')
        expect(target.get('CF-Connecting-IP')).toBe('203.0.113.77')
    })
    ...
    it('does not forward Docker private IP to the backend', () => {
        ...
        expect(target.get('X-EJP-Client-IP')).toBeNull()
    })
})
