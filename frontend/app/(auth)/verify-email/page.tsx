...
import { otpCodeSchema, OtpCodeValues } from "@/features/auth/schemas/reset-password.schema"
...
function VerifyEmailForm() {
    ...
    async function onSubmit(values: OtpCodeValues) {
        if (!email) {
    ...
    if (!email) {
        return (
            <Card className={cardClass}>
                <CardHeader>
                    <CardTitle className="text-[#0b1f1c]">Verify Email</CardTitle>
                    <CardDescription className="text-[#5a736c]">
                        Missing email address. Start again from registration.
                    </CardDescription>
                </CardHeader>
                <CardFooter>
    ...
    return (
        <Card className={cardClass}>
            <CardHeader className="space-y-1.5">
                <CardTitle className="text-2xl tracking-tight text-[#0b1f1c]">Verify Email</CardTitle>
                <CardDescription className="text-[#5a736c]">
                    Enter the 6-digit code we sent to <span className="font-medium text-[#0b1f1c]">{email}</span>.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <form
                    onSubmit={form.handleSubmit(onSubmit, () => triggerShake())}
