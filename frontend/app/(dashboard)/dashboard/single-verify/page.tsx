...
import { SingleVerifyForm, VerificationResult } from "@/app/(dashboard)/_components/single-verify/form"
    ...
    useEffect(() => {
        ...
        const handleJobUpdate = (event: any) => {
            ...
            if (detail.job_id === result.job_id) {
                logger.info("Real-time job update received:", detail.data);
                setResult(detail.data);
            }
        };

        window.addEventListener('ws:job_update' as any, handleJobUpdate);
        return () => window.removeEventListener('ws:job_update' as any, handleJobUpdate);
    }, [result]);
