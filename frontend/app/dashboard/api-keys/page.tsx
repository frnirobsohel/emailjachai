...
export default function ApiKeysPage() {
    ...
    const copyToClipboard = async (id: number, currentKey?: string) => {
        let textToCopy = currentKey;
        if (!textToCopy) {
            try {
                const res = await ApiClient.get(`/user/keys/show?id=${id}`);
                if (res.status === 'success') {
                    textToCopy = (res.data as any).api_key;
                }
            } catch (e) {
                console.error("Failed to fetch full key:", e);
                alert("Failed to retrieve key for copying.");
                return;
            }
        }
