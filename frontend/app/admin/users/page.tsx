        ...
        <div className="flex-1 space-y-4">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    ...
                    <Button className="bg-[#0f172b] hover:bg-[#0f172b]/90 text-white">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-sm mx-4 p-6 space-y-4">
                        ...
                        <div className="flex gap-2 pt-1">
                            ...
                            <button
                                onClick={async () => { if (!loginAsTarget) return; try { const response = await fetch('/next-api/auth/impersonate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: loginAsTarget.id }) }); const result = await response.json(); if (result.status === 'success') { window.location.href = '/dashboard'; } else { alert(result.message || 'Failed to login as user'); } } catch (err) { console.error('Impersonation failed', err); alert('An error occurred during impersonation.'); } finally { setLoginAsTarget(null); } }}
