...
var domainLimiters sync.Map // domain -> *rate.Limiter

func domainRPS(free bool) (float64, int) {
	if free {
		return envFloat("SMTP_FREE_DOMAIN_RPS", defaultFreeDomainRPS), envInt("SMTP_FREE_DOMAIN_BURST", defaultFreeBurst)
	}
	return envFloat("SMTP_DOMAIN_RPS", defaultDomainRPS), envInt("SMTP_DOMAIN_BURST", defaultDomainBurst)
}
...
// waitDomainUntil is a convenience wrapper bounded by a wall deadline.
func waitDomainUntil(domain string, isFree bool, deadline time.Time) error {
	ctx, cancel := context.WithDeadline(context.Background(), deadline)
	defer cancel()
	return WaitDomainRateLimit(ctx, domain, isFree)
}
